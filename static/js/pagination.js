		...
		function drawLinks() {
			...
			var appendItem = function(page_id, appendopts) {
				...
				if (page_id == current_page) {
					var lnk = $("<span class='current' style='padding:0 10px;'>" + (appendopts.text)
							+ "</span>");
				} else {
					var lnk = $("<a  style='padding:0 10px;'>" + (appendopts.text) + "</a>").bind(
			...
			if (opts.display_msg) {
				if(!maxentries){
					panel
						.append('<div class="pxofy">没有找到搜索结果。</div>');
				}else{
				panel
